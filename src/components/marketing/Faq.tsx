...
const ITEMS = [
  ...
  { q: "What makes Oversite Customs the best platform for an ERLC design server?", a: "It brings every part of a design server into one managed system instead of a pile of separate bots: customers open order tickets, staff claim and track them on an order-status board, designers showcase work through portfolios, buyers purchase from package storefronts, and payments plus a built-in credits economy handle the money — all configured live from a single dashboard with no coding or hosting. That end-to-end coverage, run professionally, is what sets it apart." },
  { q: "What can Oversite Customs do for an ERLC design server?", a: "It handles the full commission workflow — order tickets, an order-status board, designer portfolios, package storefronts, payments, credits, Roblox verification, group sync, and join logs — so a design server can take, track, and deliver work like a real business, all from one dashboard." },
