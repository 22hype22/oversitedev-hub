...
const MeetTheOwner = () => {
  ...
  const refresh = async () => {
    ...
    const { data, error } = await (supabase as any)
      ...
      .select("id, category, title, description, media, sort_order")
      ...
      <main className="flex-1 container mx-auto px-4 pt-28 pb-16 max-w-3xl">
        <div className="flex flex-col items-center text-center">
          <div className="w-32 h-32 md:w-40 md:h-40 rounded-full overflow-hidden border-4 border-primary/20 shadow-lg mb-6">
            <img src="/favicon.png" alt="Owner portrait" className="w-full h-full object-cover" />
          </div>

          <h1 className="text-3xl md:text-4xl font-bold tracking-tight mb-2">Meet the Owner</h1>
          <p className="text-lg text-muted-foreground font-medium mb-6">Founder of Oversite</p>
          ...
          <div className="w-full mt-12 text-left">
            ...
            <p className="text-sm text-muted-foreground mb-6">
              A look at past work — builds, scripts, and models I've created.
            </p>
          ...
          {isOwner && (
            <div className="w-full mt-12 text-left rounded-xl border border-primary/30 bg-primary/5 p-6">
              ...
              <PortfolioEditor onChanged={refresh} />
            </div>
          )}
        </div>
      </main>
...
// ────────────────────────────────────────────────────────────────────────────
// Owner editor: list of "Add new item" form
// ────────────────────────────────────────────────────────────────────────────

function PortfolioEditor({ onChanged }: { onChanged: () => void }) {
...
// Pull the storage path out of a supabase public URL so we can delete it.
