...
BEGIN
  ...
  -- Peak servers: number of guilds in the live status snapshot.
  SELECT COALESCE(jsonb_array_length(brs.guilds), 0)::numeric
  INTO _peak_servers
  FROM bot_runtime_status brs
  WHERE brs.bot_id = _bot_id;

  -- Peak members: sum of member_count from the persistent per-guild table,
  -- falling back to the snapshot if the table is empty.
  SELECT COALESCE(SUM(COALESCE(member_count, 0))::bigint, 0)
  INTO _peak_members
