...
BEGIN
  ...
  -- Current snapshot for peak servers/members (no history table yet — use latest heartbeat).
  SELECT
  ...
  SELECT
    d.d AS day,
    COALESCE(c.commands_count, 0)::bigint AS commands_count,
    COALESCE(c.messages_count, 0)::bigint AS messages_count,
    COALESCE(c.errors_count, 0)::bigint   AS errors_count,
    _peak_servers                          AS avg_active_servers,
    _peak_members                          AS max_member_count
  FROM days d
