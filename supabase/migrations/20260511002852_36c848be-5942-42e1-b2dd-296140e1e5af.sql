CREATE OR REPLACE FUNCTION public.create_support_access_code(_expires_in_hours integer, _notes text DEFAULT NULL::text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  END IF;
  IF _expires_in_hours IS NULL OR _expires_in_hours < 0 OR _expires_in_hours > 24 * 30 THEN
    RETURN jsonb_build_object('ok', false, 'error', 'Expiry must be between 0 (never) and 30 days.');
  END IF;
  ...
  IF _expires_in_hours = 0 THEN
    _expires := 'infinity'::timestamptz;
  ELSE
