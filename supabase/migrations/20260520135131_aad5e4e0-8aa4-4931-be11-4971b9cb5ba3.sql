...
BEGIN
  ...
  IF _bot_ids IS NOT NULL AND array_length(_bot_ids, 1) IS NOT NULL THEN
    -- Only transfer bots still owned by the original owner. Lock them so a
    -- concurrent transfer/cancel/update cannot partially race this operation.
    ...
    IF array_length(_bot_ids, 1) IS NOT NULL THEN
      -- Current app logic treats the Web Dashboard add-on as an account-wide
      -- entitlement derived from bot_orders.addons. If the previous owner had
      -- dashboard access, stamp that add-on onto the transferred bot rows so
      -- the recipient can open the dashboard immediately after ownership moves.
      SELECT EXISTS (
        SELECT 1
          FROM public.bot_orders
         WHERE user_id = _old_owner
      ...
      UPDATE public.bot_orders
         SET user_id = _new_owner,
             ...
             END,
             updated_at = now()
       WHERE id = ANY(_bot_ids)
         AND user_id = _old_owner;
