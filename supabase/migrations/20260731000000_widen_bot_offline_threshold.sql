-- Widen the heartbeat staleness window from 60s to 120s.
--
-- Bots heartbeat every 30s (HEARTBEAT_INTERVAL_MS), but the offline threshold
-- was 60s — only two heartbeats of slack. A single delayed or dropped
-- heartbeat write (edge-function cold start, transient network blip, Railway
-- CPU throttle) pushed the gap past 60s and made healthy bots flap to
-- "offline" on the dashboard. 120s tolerates up to four missed heartbeats
-- before flipping, which kills the false offlines while still surfacing a
-- genuinely dead bot within ~2 minutes.
...
BEGIN
  ...
  IF _row.last_heartbeat_at IS NOT NULL THEN
    ...
    -- Heartbeat-first: if the worker has pinged within the last 120s, the bot
    ...
    ELSIF _row.status IN ('online','starting','stopping','updating') THEN
      -- Stored status claims live, but no heartbeat in >120s → effectively offline.
      _effective := 'offline';
      _stale := true;
    END IF;
  END IF;
