...
BEGIN
  ...
  FOR _g IN SELECT * FROM jsonb_array_elements(_guilds) LOOP
    ...
    -- Treat 0 as "unknown" (Discord cache often reports 0 before READY) and preserve previous value.
    IF _mc IS NULL OR _mc = 0 THEN
      ...
      IF _existing IS NOT NULL AND _existing > 0 THEN
        _mc := _existing;
      END IF;
    END IF;
...
-- Make the metrics RPC prefer bot_active_guilds.member_count (the persistent source of truth)
-- and fall back to the snapshot if the table is empty.
...
BEGIN
  ...
  -- Prefer the bot_active_guilds table (persistent member_count per guild).
  SELECT COUNT(*)::numeric, COALESCE(SUM(COALESCE(member_count, 0))::bigint, 0)
  INTO _peak_servers, _peak_members
  FROM bot_active_guilds
  WHERE bot_id = _bot_id;

  -- Fall back to the snapshot if no guilds tracked.
  IF COALESCE(_peak_servers, 0) = 0 THEN
    SELECT
      COALESCE(jsonb_array_length(brs.guilds), 0)::numeric,
      COALESCE((
        SELECT SUM(COALESCE((g->>'member_count')::bigint, 0))
