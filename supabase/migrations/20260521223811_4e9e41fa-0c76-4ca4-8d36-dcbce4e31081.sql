...
BEGIN
  ...
  -- ── Per-bot team membership migration ────────────────────────────────
  -- Remove the recipient's team memberships for the bots they now own
  -- (they shouldn't appear as members of themselves).
  IF _bot_ids IS NOT NULL AND array_length(_bot_ids, 1) IS NOT NULL THEN
    DELETE FROM public.dashboard_team
     WHERE bot_id = ANY(_bot_ids)
       AND (
         member_user_id = _new_owner
         OR lower(member_email) = lower(COALESCE(_caller_email, ''))
       );

    -- Re-key ownership of remaining per-bot team rows so the new owner sees
    -- the existing roster for each transferred bot.
    UPDATE public.dashboard_team
       SET owner_user_id = _new_owner,
           updated_at = now()
     WHERE bot_id = ANY(_bot_ids)
       AND owner_user_id = _old_owner;

    -- Add the previous owner as a viewer on each transferred bot.
    IF _old_owner_email IS NOT NULL AND _old_owner <> _new_owner THEN
      FOREACH _bid IN ARRAY _bot_ids LOOP
        INSERT INTO public.dashboard_team
          (owner_user_id, bot_id, member_email, member_user_id, role, invited_by, invited_at, accepted_at)
        VALUES
          (_new_owner, _bid, _old_owner_email, _old_owner, 'viewer', _new_owner, now(), now())
        ON CONFLICT (bot_id, lower(member_email))
  ...
  -- Clear the transfer token on the originating row so it can't be reused.
  UPDATE public.dashboard_team
     SET transfer_token = NULL,
         transfer_requested_at = NULL,
         transfer_requested_by = NULL,
