-- Team members could save a bot's config (RLS checks has_bot_team_perm) but
-- the "notify the bot" RPCs still only accepted the owner, an admin, or
-- support access, so the dashboard showed "Saved, but failed to notify bot:
-- Not allowed." Let the same team permission through on all three.
...
BEGIN
  ...
  IF _bot_owner <> _user_id
     ...
     AND NOT public.has_bot_team_perm(_user_id, _bot_id, 'edit_bot_config') THEN
...
-- Start, stop, restart, and update are settings-level actions.
...
BEGIN
  ...
  IF _bot_owner <> _user_id
     AND NOT public.has_role(_user_id, 'admin'::app_role)
     AND NOT public.has_support_access(_user_id, _bot_owner)
     AND NOT public.has_bot_team_perm(_user_id, _bot_id, 'manage_settings') THEN
    RETURN jsonb_build_object('ok', false, 'error', 'Not allowed.');
  END IF;
