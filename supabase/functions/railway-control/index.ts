...
type Action = "start" | "restart" | "redeploy";
const ALLOWED: Action[] = ["start", "restart", "redeploy"];
...
async function ensureRunning(serviceId: string, environmentId: string) {
  await railway(
    `mutation($serviceId: String!, $environmentId: String!) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Authorization: owner, team edit_bot_config, or admin
    let authorized = bot.user_id === userId;
    if (!authorized) {
      const { data: hasPerm } = await admin.rpc("has_team_perm", {
        _user_id: userId,
        _owner_id: bot.user_id,
        _perm: "edit_bot_config",
      });
      authorized = Boolean(hasPerm);
    }
    if (!authorized) {
    ...
    try {
      ...
      await redeploy(bot.railway_service_id, environmentId);
