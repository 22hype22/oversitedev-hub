// Returns the raw Discord message JSON (embeds, components, content, etc.)
// for a given (bot_id, channel_id, message_id), using the bot's stored
// DISCORD_TOKEN. The dashboard's ticket-panel editor uses this to faithfully
// reconstruct the live panel layout for messages it has only discovered via
// backfill (where the original builder state was never saved).
//
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!dRes.ok) {
      ...
      return json(502, {
        error: `Discord API ${dRes.status}: ${text.slice(0, 200)}`,
      });
    }
    const message = await dRes.json();
    return json(200, { ok: true, exists: true, message });
  } catch (e) {
    console.error("bot-fetch-message error", e);
