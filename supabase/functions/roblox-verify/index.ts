// Roblox OAuth verification for Oversite bots.
//
// Entry points on one function:
//   POST { action: "start",  bot_id, guild_id, discord_user_id }  (bot, x-worker-token)
//        → if this Discord account has already verified through ANY Oversite
//          bot, copies that link to this bot and returns { verified: true };
//          otherwise creates a one-time state row and returns the Roblox
//          authorize URL.
//   POST { action: "recall", bot_id, guild_id, discord_user_id }  (bot, x-worker-token)
//        → the memory alone, for a member who has just joined: returns the
//          remembered link if there is one, never opens a login.
//   POST { action: "lookup", bot_id, discord_user_id }             (bot, x-worker-token)
//        → is this member verified in THIS bot, and as whom.
...
//
// Which Roblox OAuth app is used: the bot's own, if its dashboard has a client
// id and secret; otherwise Oversite's. Roblox gates creating an OAuth app
// behind ID verification, so some communities cannot get one — they still get
// verification, and their members see "Oversite" on the Roblox consent screen.
// The callback must exchange the code with the same app that opened the
// session, so the session records which bot's credentials it used.
//
// Every app registers THIS function's URL as the OAuth redirect URI:
...
// The platform bot: its dashboard holds Oversite's own Roblox OAuth app, which
// the website already uses for account linking.
...
type OAuthApp = { clientId: string; clientSecret: string; oauthBotId: string; shared: boolean };

/** The OAuth app to verify this bot's members with: its own, else Oversite's. */
async function resolveOAuthApp(botId: string, cfg?: Record<string, any> | null): Promise<OAuthApp | null> {
  const own = cfg === undefined ? await getConfig(botId) : cfg;
  const ownId = String(own?.roblox_client_id ?? "").trim();
  const ownSecret = String(own?.roblox_client_secret ?? "").trim();
  if (ownId && ownSecret) return { clientId: ownId, clientSecret: ownSecret, oauthBotId: botId, shared: false };
  if (botId !== SITE_BOT_ID) {
    const site = await getConfig(SITE_BOT_ID);
    const id = String(site?.roblox_client_id ?? "").trim();
    const secret = String(site?.roblox_client_secret ?? "").trim();
    if (id && secret) return { clientId: id, clientSecret: secret, oauthBotId: SITE_BOT_ID, shared: true };
  }
  return null;
}

/** Credentials for a session already opened — the app that opened it. */
async function appForSession(sess: { bot_id: string; oauth_bot_id?: string | null }): Promise<OAuthApp | null> {
  const oauthBotId = String(sess.oauth_bot_id ?? "").trim();
  if (oauthBotId) {
    const cfg = await getConfig(oauthBotId);
    const id = String(cfg?.roblox_client_id ?? "").trim();
    const secret = String(cfg?.roblox_client_secret ?? "").trim();
    if (id && secret) return { clientId: id, clientSecret: secret, oauthBotId, shared: oauthBotId !== String(sess.bot_id) };
  }
  // Sessions from before oauth_bot_id existed, or a bot that has since
  // changed its keys: fall back to whatever this bot resolves to now.
  return resolveOAuthApp(String(sess.bot_id));
}

async function authorizeUrl(app: OAuthApp, state: string): Promise<string> {
  return (
    `${ROBLOX_AUTHORIZE}?client_id=${encodeURIComponent(app.clientId)}` +
...
/** Check the worker token names this bot. Returns null when it does. */
async function workerMismatch(req: Request, botId: string): Promise<Response | null> {
  const token =
  ...
  const { data: lookup, error: lookupErr } = await admin.rpc("_worker_token_lookup", { _token: token });
...
// ── Memory ───────────────────────────────────────────────────────────────────
// A link in roblox_verifications only ever comes from a completed Roblox OAuth
// login, so it proves that Discord account controls that Roblox account. That
// proof does not expire at a server border.

type Remembered = { roblox_id: string; roblox_username: string };

/** The newest link this Discord account has made through any OTHER bot. */
async function remembered(discordUserId: string, exceptBotId: string): Promise<Remembered | null> {
  const { data } = await admin
    .from("roblox_verifications")
    .select("roblox_id, roblox_username, verified_at")
    .eq("discord_user_id", discordUserId)
    .neq("bot_id", exceptBotId)
    .order("verified_at", { ascending: false })
    .limit(1)
    .maybeSingle();
  if (!data?.roblox_id) return null;
  return { roblox_id: String(data.roblox_id), roblox_username: String(data.roblox_username ?? "") };
}

/** Record a link for this bot and ask the bot to apply nickname + roles. */
async function storeAndApply(botId: string, guildId: string, discordUserId: string, link: Remembered) {
  ...
  // requested_by/user_id must be set on a command.
  const { data: order } = await admin.from("bot_orders").select("user_id").eq("id", botId).maybeSingle();
...
const reuseAllowed = (cfg: Record<string, any> | null) => cfg?.reuse_verifications !== false;

// ── POST start: a member clicked Verify ─────────────────────────────────────
async function handleStart(req: Request): Promise<Response> {
  ...
  const denied = await workerMismatch(req, botId);
  if (denied) return denied;

  const cfg = await getConfig(botId);

  // Already proven elsewhere: apply it here and skip the login.
  if (reuseAllowed(cfg)) {
    const link = await remembered(discordUserId, botId);
    if (link) {
      await storeAndApply(botId, guildId, discordUserId, link);
      return json({ ok: true, verified: true, remembered: true, ...link });
    }
  }

  const app = await resolveOAuthApp(botId, cfg);
  if (!app) return json({ error: "Roblox verification is not set up." }, 400);

  // One-time state (uuid). Old sessions for this member are cleared first.
  const state = crypto.randomUUID();
  await admin.from("roblox_verify_sessions").delete().eq("bot_id", botId).eq("discord_user_id", discordUserId);
  const { error: insErr } = await admin.from("roblox_verify_sessions").insert({
    state,
    bot_id: botId,
    guild_id: guildId,
    discord_user_id: discordUserId,
    oauth_bot_id: app.oauthBotId,
  });
  if (insErr) return json({ error: `Could not start verification: ${insErr.message}` }, 500);

  return json({ ok: true, verified: false, url: await authorizeUrl(app, state), shared_app: app.shared });
}

// ── POST recall: a member just joined — do we already know them? ────────────
async function handleRecall(req: Request): Promise<Response> {
  const body = await req.json().catch(() => ({}));
  const botId = String(body.bot_id ?? "");
  const guildId = String(body.guild_id ?? "");
  const discordUserId = String(body.discord_user_id ?? "");
  ...
  const cfg = await getConfig(botId);
  if (!reuseAllowed(cfg)) return json({ ok: true, verified: false, reason: "reuse_off" });

  // Verified in this bot before (came back after leaving): apply that.
  const { data: own } = await admin
    ...
    .eq("discord_user_id", discordUserId)
    .maybeSingle();
  const link = own?.roblox_id
    ? { roblox_id: String(own.roblox_id), roblox_username: String(own.roblox_username ?? "") }
    : await remembered(discordUserId, botId);
  if (!link) return json({ ok: true, verified: false });

  await storeAndApply(botId, guildId, discordUserId, link);
  return json({ ok: true, verified: true, remembered: !own?.roblox_id, ...link });
}
...
async function handleSiteStart(req: Request): Promise<Response> {
  ...
  const app = await resolveOAuthApp(SITE_BOT_ID);
  if (!app) return json({ error: "Roblox login is not set up on the site yet." }, 500);
  ...
  return json({ url: await authorizeUrl(app, state) });
}
...
async function handleCallback(url: URL): Promise<Response> {
  ...
  const { data: sess } = await admin
    ...
    .select("bot_id, guild_id, discord_user_id, expires_at, site_user_id, return_to, oauth_bot_id")
    .eq("state", state)
    .maybeSingle();
  ...
  const app = await appForSession(sess as any);
  if (!app) {
  ...
  await storeAndApply(String(sess.bot_id), String(sess.guild_id ?? ""), String(sess.discord_user_id), {
    roblox_id: robloxId,
    roblox_username: robloxUsername,
  });
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (body?.action === "recall") return await handleRecall(req);
