...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const winners = Math.max(1, Math.min(100, Number(body.winners || 1)));

    if (!botId) return json(400, { error: "botId required" });
    if (!prize) return json(400, { error: "prize required" });
    if (!durationMinutes || durationMinutes < 1) {
      return json(400, { error: "duration_minutes must be > 0" });
    }

    const admin = createClient(SUPABASE_URL, SERVICE_ROLE);

    // Verify the caller owns the bot being configured.
    ...
    // Look up the utilities bot owner so the row passes RLS checks.
    ...
    console.error("enqueue-giveaway-start error", e);
